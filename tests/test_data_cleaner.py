...
import unittest
from pandas.api import types as pdt2
import numpy as np
...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        self.assertEqual(df['name'][3], " Carol  ")
        
        self.assertEqual(result['name'][0], "Alice")
        self.assertEqual(result['name'][3], "Carol")
        
        expected = pd.Series(["SCL", "LPZ", "SCL", "LPZ"], name="city")
        pdt.assert_series_equal(result["city"], expected)
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        """
        df = make_sample_df()
        #Se llena el valor faltante de 'age' con un número para evitar problemas de lógica antes de probar remove_outliers_iqr
